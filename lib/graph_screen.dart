...
class _HealthTempState extends State<HealthTemp> {
  ...
  Future fetchData() async {
    final response =
        await http.get(Uri.parse('http://localhost/tracking/gettemp.php'));

    if (response.statusCode == 200) {
      final List<dynamic> jsonData = json.decode(response.body);
      for (var data in jsonData) {
        String Idcard = data['id_card'];
        double HeartValue = double.parse(data['heart_value']);
        print('Date: $Idcard, Temperature: $HeartValue');
// น าข้อมูลไปใช้งานต่อได้ตามที่คุณต้องการ
        return double.parse(data['heart_value']);
      }
    } else {
